...
The most important model class are generalized additive models (GAM) \citep{gam-book}. Here the conditional distribution of the response variable is assumed to follow an exponential family distribution. Then, the expected response $\Y|\X$ is modeled given the observed value $\x$ using a link function $g$ as
\begin{equation}
    g(\mathbb{E}(\Y|\X=\x))=f(\x).
\end{equation}
The function $f(\cdot)$ is called an additive predictor, and consists of the additive effects of the single predictors,
...
\begin{enumerate}
    ...
    \item Find best step length for $a^{[m]}$ by a line search:
        ...
        \end{equation*}
    \item Update $f^{[m]}(\cdot)\gets f^{[m-1]}(\cdot)+a^{[m]}\cdot h^{[m]}(\cdot)$.
...
\end{equation}
We select the best-fitting base-learner $h_{j^{[m]}}^{[m]}$ based on the residual sum of squares of the base-learner fit
\begin{equation}
    j^{[m]}=\argmin_{j\in\{1,2,\ldots,p\}}\sum_{i=1}^N \left(u_i-\hat{h}_j^{[m]}\right).
\end{equation}
...
\end{equation}
Note that any base-learner $h_j$ can be selected in multiple iterations. The partial effect of the variable $x_j$ is the sum of the estimated corresponding base learner in all iterations where it was selected, i.e.,
\begin{equation*}
    f_j(x_j)=\sum_{m=1}^{m_{\text{stop}}}\nu\cdot\hat{h}_j(x_j)\indicator\left(j^{[m]}=j\right).
\end{equation*}
...
\end{enumerate}
\end{algorithm}
\subsection{Data-driven variable selection}
