...
\begin{equation}
    R(f^{[m+1]}_{\theta_k}).
\end{equation}
We calculate the gain in the loss function, which we denote $\rho_{\theta_k}$, by
\begin{equation}
    \rho_{\theta_k}=R(f^{[m]})-R(f^{[m+1]}_{\theta_k}).
\end{equation}
If we now compare the gain in loss function value, $\rho_{\theta_k}$, across each parameter $k=1,2,\ldots,K$, we can find out which parameter leads to the best increase. We choose that one, and incorporate only that one into the full boosting model, i.e.,
\begin{equation}
    f^{[m+1]}=f^{[m+1]}_{\theta_k}=f^{[m]}+\nu\cdot\hat{h}_{\theta_k}(\cdot).
\end{equation}
We choose the base learner for each component by comparing their residual sum of squares with respect to the negative gradient vector, which we called the inner loss. This is, however, not the same loss function that is used to choose between parameters. This might be problematic. Therefore, \citet{thomas2018} propose using the loss function also for choosing between component-wise learners. They call this the ``outer loss.'' In that case, instead of the usual procedure of choosing the component-wise learner for each parameter which minimizes the RSS, i.e.,
\begin{equation}
    j^*=\argmin_{j} \sum_{i=1}^N (u_{k,i}-(f^{[m]}(x_i)+\hat{h}(\x_i)))^2
\end{equation}
we now choose the component-wise learner for each parameter which minimizes the outer loss function, i.e., 
\begin{equation}
    j^*=\argmin_{j} \sum_{i=1}^N \rho(y_i,f(x_i)).
\end{equation}
The individual component-wise learners are still estimated by their usual method, e.g., by linear least squares if using simple linear regression base learners.
