...
Currently, the classifier is $F_1(\cdot)=\hat{\alpha}^{[1]}\hat{h}^{[1]}(\cdot)$.
...
In the next iteration, we again apply a weak learner which minimizes the weighted sum of the observations, and we reweight the observations accordingly, in the same manner as before.
...
\begin{equation*}
    \hat{F}(\cdot)=F^{[\mstop]}(\cdot)=\sum_{m=1}^{\mstop}\hat{\alpha}^{[m]}\hat{h}^{[2]}(\cdot),
\end{equation*}
...
This algorithm has been implemented in R as \verb|mboost| \citep{mboost, mboost1, mboost2}.
...
\end{equation*}
Using boosting, we may obtain estimates of $\CV(\boldsymbol{m})$ for all $\boldsymbol{m}$ by fixing all but one of the parameters and perform a typical boosting run.
