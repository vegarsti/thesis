...
We are also interested in the hazard function.
...
The reasons for this will become clear.
...
Assume now that the process will reach 0 with certainty, such that $P(T=\infty)$ \eqref{eq:T-is-infinity} is 0.
Again, this will happen if the drift is non-positive.
Let us consider a case where $\mu$ is strictly negative, and not zero.
...
\begin{equation*}
    Y(t)\approx y_0+\mu t.
\end{equation*}
Consequently, the first hitting time will be nearly a deterministic function of $y_0$ and $\mu$, such that
\begin{equation*}
...
Note that we may let $\x$ and $\z$ share none, some, or all elements.
...
A few software packages for doing this exist.For \verb|R| \citep{Rlang}, the \verb|threg| package \citep{threg}.
...
Since we have a parametric expression for the survival function $S(t)$ \eqref{eq:ig-surv}, we can construct parametric estimates of the survival probability at time $t$, by plugging in estimates the parameters, $\hat{y}_0$ and $\hat{\mu}$.
Since $\sigma^2$ is 1, we remove it from the expression as well.
