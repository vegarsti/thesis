...
While these events are a relapse and a birth, respectively, we will collectively refer to them as deaths, and hence the time to the event happens we call time-to-death.
...
In addition, if there is perfect Cox structure given $x_1$ alone, and perfect Cox structure given $x_2$ alone, one almost never has a Cox model $(x_1,x_2)$.
...
\begin{equation*}
    \hz(t)=f(t)/S(t).
\end{equation*}
If we insert the inverse Gaussian versions of $f(t)$ and $S(t)$, we would get a rather unruly expression.
However, we can make some comments on its shape as it relates to different values for $y_0$ and $\mu$.
